#!/usr/bin/env bun
// PostHog launcher — a single-node PostHog analytics suite on DigitalOcean.
//
...
//
// COLORS_PAR_PROFILE is the exception — posthog refuses to run when it is
// set. The profile names this project's work directory, OpenTofu state key
// and cloud resources, and overriding it from the environment can only point
// posthog at another project's state.
//
...
// own: validation, the graph and the steps all live in the `package-posthog-red`
...
//
// "package-posthog-red" is null before this repository has been pushed and
...
// rewrites null to "github:getcolors/posthog#<sha>" in place.
//
...
const PINS = {
  "package-posthog-red": "github:getcolors/posthog#ce05ed2572d80a215c5dcf95e0422ec926916b16",
...
// add-deps. POSTHOG_LIB_ROOT overrides everything with a working tree: point
// it at a posthog checkout whose dependencies are installed, and the copied
// payload runs those sources instead of any pin.
 ...
 *
 * The payload's canonical home is skills/package-posthog-red/ inside the
 * posthog repository, whose working tree is the point: resolving a pinned
 * copy from the cache would quietly test the pinned commit instead of the
 * edits under test.
 */
function checkoutRedDir() {
  const candidate = join(import.meta.dir, "..", "..", "red");
  const manifest = readManifest(candidate);
  return manifest?.name === "package-posthog-red-dev" ? candidate : null;
}
...
function installToCache() {
  ...
  const root = join(home, "package-posthog-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-posthog-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let posthog;
try {
  posthog = await import("package-posthog-red");
} catch (err) {
  ...
  const libRoot = process.env.POSTHOG_LIB_ROOT;
  ...
  if (libRoot) {
    ...
    posthog = await importWorkingTree(overrideDir, "POSTHOG_LIB_ROOT");
    ...
    posthog = await importWorkingTree(redDir, "checkout");
  } else if (!PINS["package-posthog-red"]) {
    console.error(
      "red: this launcher carries no posthog pin.\n" +
        "It has not been stamped by `bb pin`, which cannot run until the " +
        "posthog repository has been pushed.\n" +
    ...
    // not to a path.
    posthog = await import(Bun.resolveSync("package-posthog-red", dir));
  }
}

export const run = posthog.run;
if (import.meta.main) await posthog.exec();
